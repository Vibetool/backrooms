...
A.register({
  ...
  anim: {
    gait: 'none', recoil: 0.06,
    // 依据：没有身体/腿部描述，不走标准步态；自我了结时用多彩粒子爆炸代替原来的 fall:'fade' 渐隐（见下方
    // onFrame），corpseSec 沿用原来的短时长，配合爆炸一瞬间"一下子就没了"的观感
    ...
    onFrame(e, dt, api, u) {
      // 依据："超出范围约 60 米时爆炸，释放彩纸"——auto() 在实体刚死的那一帧把 u.deadAt 记成当前时刻，
      // 借这个时机抓住"刚死亡"的唯一一帧，用 A.fx.burst 炸出一团多色彩纸代替单纯的渐隐消失（ENGINE_PLAN
      // M1 回填：警告风筝 self-destruct → A.fx.burst 彩纸）
      if (e.dead && u.deadAt === api.time) {
        A.fx.burst(e.x, e.y + e.h * 0.5, e.z, {
          color: [0xff5252, 0xffd740, 0x40c4ff, 0x69f0ae, 0xff4fd8],   // 彩纸取多种鲜艳色，非设定精确色值
          count: 30, speed: 2.6, life: 1.2, gravity: 2.2, up: 0.5, spread: 0.1,
          flashSec: 0.2, radius: 0.35,
        });
      }
    },
  },
...
// - 已实现（2026-09-14，ENGINE_PLAN M1 回填）："爆炸，释放彩纸"——自我了结时用 A.fx.burst 炸出一团多色
//   粒子代替原来的 fall:'fade' 渐隐（anim.onFrame，抓 u.deadAt 命中死亡第一帧触发一次）。仍是简化效果：
//   没有真正的纸片形状/翻转物理，只是彩色粒子爆炸，颜色和数量非设定精确值。
